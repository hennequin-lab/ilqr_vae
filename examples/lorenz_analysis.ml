...
let do_for prefix prms =
  ...
  |> Array.iteri ~f:(fun i data ->
    ...
    then (
      ...
      let u =
        let mu_u = Model.posterior_mean ~prms data |> AD.unpack_arr in
        AA.save_txt
          ~out:(in_dir Printf.(sprintf "%s_extrapolation_posterior_u_%i" prefix i))
          mu_u;
        let mask = AA.(ones [| 50; 1 |] @= zeros [| 50; 1 |]) in
        AD.pack_arr AA.(mask * mu_u)
      in
      let module D = Dynamics.Integrate (D) in
      let z = D.integrate ~prms:prms.dynamics ~n:setup.n ~u:(AD.expand0 u) in
      let o = L.pre_sample ~prms:prms.likelihood ~z:(AD.squeeze0 z) in
      AA.save_txt
...
let _ =
  let module D = Dynamics.Integrate (D) in
  let u =
    AD.pack_arr AA.(gaussian ~sigma:5. [| 1; setup.m |] @= zeros [| 9999; setup.m |])
  in
  let z = D.integrate ~prms:prms_final.dynamics ~n:setup.n ~u:(AD.expand0 u) in
